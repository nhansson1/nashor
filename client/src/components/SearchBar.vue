...
const handleSubmit = async (e: Event) => {
    ...
    router.push({
        name: "summoner",
        params: { region: selectedRegion.value, gameName: profile.gameName, tagLine: profile.tagLine },
    });
};
</script>
...
<template>
    <form @submit="handleSubmit" class="searchbar">
        <select @change="handleChange" name="region" id="region" class="searchbar__region">
            <option v-for="region in regions" :value="region.value">
                {{ region.name }}
            </option>
        </select>
        <input v-model="searchModel" placeholder="Search a Summoner" type="text" class="searchbar__input" />
    </form>
</template>
