...
if (props.summonerData.ranks.length !== 2) {
  ...
  props.summonerData.ranks = [
    props.summonerData.ranks[0],
    props.summonerData.ranks[1],
  ].map((rankEntry, index, arr) => {
    if (rankEntry) return rankEntry;

    const otherRank = index === arr.length - 1 ? arr[0] : arr[1];

    if (!otherRank)
      return {
        queueType: index === 0 ? "RANKED_SOLO_5x5" : "RANKED_FLEX_SR",
        ...rankDefaults,
      };

    return {
      queueType:
        otherRank.queueType === "RANKED_SOLO_5x5"
          ? "RANKED_FLEX_SR"
          : "RANKED_SOLO_5x5",
      ...rankDefaults,
    };
...
<template>
  <div class="summoner-profile">
    <SummonerContainer
      :profile-icon-id="props.summonerData.summoner.profileIconId"
      :riot-id="riotId"
    />
